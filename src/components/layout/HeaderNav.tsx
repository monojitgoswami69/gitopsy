...
export function HeaderNav() {
  ...
  return (
    <header className="w-full shrink-0 z-40 border-b-[3px] border-black bg-[#F4EFE6]/90 backdrop-blur-md px-4 sm:px-6 py-3 shadow-[0_3px_0_0_#000] flex items-center justify-between relative">
      {/* Logo & Tagline */}
      <Link href="/" className="flex items-center gap-2.5 select-none group z-10">
        <div className="relative size-9 shrink-0 group-hover:scale-105 transition-transform">
          <Image
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-bold text-gray-600 hidden lg:inline">
      ...
      <div className="flex items-center gap-3 z-10">
        {isAuthenticated ? (
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            >
              <LogOut className="size-5 sm:size-5.5 stroke-[2.5]" />
            </button>

            {/* Profile Name directly on page */}
            {displayUsername && (
              <span className="text-xs sm:text-sm font-mono font-black text-black select-none tracking-tight leading-none">
                {displayUsername}
              </span>
            )}
