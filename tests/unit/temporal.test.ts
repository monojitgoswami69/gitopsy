...
describe("Temporal Analytics — Hardened Behavior", () => {
  it("should count night commits in the 21:00-04:59 UTC window", () => {
    ...
    const result = calculateTemporalAnalytics(commits);

    // 2 of 4 commits are night (22:00 and 03:00)
    expect(result.nightCommitPercentage).toBe(50);
  });

  it("should include hour 4 (04:00-04:59) in night window", () => {
    const commits: ForensicCommit[] = [
  ...
  it("should exclude hour 5 (05:00) from night window", () => {
    const commits: ForensicCommit[] = [
  ...
  it("should handle empty commits", () => {
    const result = calculateTemporalAnalytics([]);
